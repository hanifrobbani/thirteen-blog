...
@section('container')
<h1 class="mb-5 text-2xl font-semibold">Add New Blogs</h1>

<div class="w-full">
    <form action="/blog" method="post" enctype="multipart/form-data">
        @csrf
        <div class="mb-4 w-auto">
            <input type="text" class="w-1/2 bg-white outline-none border-2 p-2 rounded-xl focus:border-blue-600 transition focus:ring-4 focus:ring-blue-200 {{ $errors->has('judul') ? 'border-red-500' : 'border-slate-300' }}" placeholder="Judul" name="judul">
            @error('judul')
        ...
        <div class="mb-4 w-auto">
            <select name="jenis_id" class="custom-select w-1/2 bg-white border-2 p-2 pr-10 rounded-xl outline-none text-slate-500 focus:border-blue-600 transition focus:ring-4 focus:ring-blue-200 {{ $errors->has('jenis_id') ? 'border-red-500' : 'border-slate-300' }}">
                ...
                <option value="1" class="text-black">Teknologi</option>
                <option value="2" class="text-black">Makanan</option>
                <option value="3" class="text-black">Budaya</option>
                <option value="4" class="text-black">Meme</option>
            </select>
            @error('jenis_id')
            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
            @enderror
        ...
        <div class="mt-7">
            ...
            <trix-editor input="x" class="block w-1/2 bg-white outline-none border-2 p-2 rounded-xl min-h-60 focus:border-blue-600 transition focus:ring-4 focus:ring-blue-200 {{ $errors->has('content') ? 'border-red-500' : 'border-slate-300' }}"></trix-editor>
            @error('content')
            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="flex justify-between w-1/2 gap-3">
            ...
            <button class="w-full text-center py-2 bg-blue-600 rounded-lg text-white font-semibold mt-4 focus:border-blue-600 transition focus:ring-4 focus:ring-blue-200 hover:bg-blue-500" type="submit">Post</button>
        </div>
    </form>
</div>
